...
void initButtonsAndButtonInterrupts(void)
{
  ...
  attachPCINT(digitalPinToPCINT(RST_BTN), i_RST_BTN, FALLING);
...
void i_RST_BTN(void)
{
  ...
  if (interrupt_time - last_interrupt_time > 800) // Longer time due to a different button type. If interrupts come faster than DEBOUNCE, assume it's a bounce and ignore
  {
    //Serial.println("\n*** RESETTED ***\n"); //
    state = GAME;
    ZeroDisplay(0); // set display to 0

    current_Time = millis(); // start timing for the game
  }
...
void i_BTN_1(void)
{
  ...
  if (interrupt_time - last_interrupt_time > 250) // 50 ms extra for the green button
  {
  if(state == STARTED) {
